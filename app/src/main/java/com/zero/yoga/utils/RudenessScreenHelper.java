...
public class RudenessScreenHelper {
    ...
    /**
     *     用于切换到横屏时候使用，需要重新初始化方法
     */

    public static void resetHorizontalDensity(Context context, float designWidth) {
        if (context == null)
            return;
        Point size = new Point();
        ((WindowManager) context.getSystemService(WINDOW_SERVICE)).getDefaultDisplay().getSize(size);
        Resources resources = context.getResources();
        resources.getDisplayMetrics().density = size.y / designWidth;
        resources.getDisplayMetrics().scaledDensity = resources.getDisplayMetrics().density;
        DisplayMetrics metrics = getMetricsOnMiui(context.getResources());
        if (metrics != null) {
            metrics.density = size.y / designWidth;
            metrics.scaledDensity = metrics.density;
        }
    }
